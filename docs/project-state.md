...
| Terraform foundation and state lifecycle await acceptance | [#8](https://github.com/toddysm/aiks/issues/8) implements the environment, backend commands, offline tests, and CI on its feature branch after the approved cluster/registry AzAPI exception. See the [operator guide](../infrastructure/aks-automatic/terraform/README.md). Review and live deployment/idempotence evidence are still required; no Azure deployment was performed. |
